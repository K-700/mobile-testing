...
class CheckoutPage
{
    ...
    public function __construct(CheckoutTester $I, Cart $cart)
     ...
     * @param TestUser $user
     */
    public function inputValidContactInfo(TestUser $user)
    {
        $I = $this->tester;
...
//        $I->swipe(0, 400, 0, 100);
     ...
     * @param Cart $cart
     */
    public function addDeliveryToCart(Cart &$cart)
    {
        $cart->addItems(new ShopItem($this->deliveryMethod->name, $this->grabDeliveryPrice()));
    }
     ...
     * @throws \Exception
     */
    public function inputPaymentType(TestUser $user, Cart &$cart = null)
    {
        $I = $this->tester;
    ...
    public function checkCouponBlock()
    {
        ...
        $I->assertContains('Ошибка при активации купона', $I->findBy($this->couponError)->text());
        $I->verticalSwipeToElement($I->findBy($this->couponCloseButton));
